...
def normalize_voice(raw_transcript: Optional[str]) -> Optional[NormalizedVoiceContext]:
    ...
    # ── Map Fact-Based Severity ──
    CRITICAL_FACTS = {"integrity", "failure", "missing", "broken", "severe", "danger", "unsafe", "cannot operate", "grounded", "active leak", "structural"}
    MODERATE_FACTS = {"leak", "leaking", "crack", "cracked", "bent", "loose", "worn", "wear", "rust", "corrosion", "corroded", "damaged", "flat", "torn", "seized", "overheating", "smoking", "grinding", "vibrating", "stuck", "low"}
    
    inferred_severity = "Normal"
    for m in MODERATE_FACTS:
        if m in clean_text:
            inferred_severity = "Moderate"
            break
            
    for c in CRITICAL_FACTS:
        if c in clean_text:
